...
void Mod_LoadTexinfo (lump_t *l)
{
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		...
		out->image = FindImage(name, it_wall);
		if (!out->image)
		{
...
void Mod_LoadSpriteModel (model_t *mod, void *buffer)
{
	...
	for (i=0 ; i<sprout->numframes ; i++)
	{
		...
		mod->skins[i] = FindImage(sprout->frames[i].name,
			it_sprite);
	}
...
{
	// Nothing to do for now. We always keep all model loaded as those are cheap nowadays
